...
public:
	 ...
	 */
	vector<Commit> getCommits() const {
		return _commits;
	}
	...
	/**
	 * @brief Elimina do vetor de utilizadores do projeto o utilizador com nome dado
	 * @param nome do utilizador a remover
	 */
	...
	void imprimeCoders();
	/**
	 * @brief Retorna chave de acesso do projeto
	 ...
	 * @param d1 - primeira limite do intervalo de datas a pesquisar
	 * @param d2 - segundo limite do intervalo de datas a pesquisar
	 * @return volume de commits
	 */
...
/**
 * @brief Classe utilizada para lançar uma exceção quando não é encontrado um projeto com o id fornecido
 * @param id - id do projeto que não foi possível encontrar
 */
class NoSuchProject {
	unsigned int id;
public:
	NoSuchProject(unsigned int i) {
		id = i;
	}
	unsigned int getId() const {
		return id;
...
template<class T>
void sortRanking(vector<Utilizador *> &vec, T *objeto, Data d1 = Data(0, 0, 0),
		Data d2 = Data(0, 0, 0)) {
	bool troca = false;
	for (unsigned int i = 0; i < vec.size() - 1; i++) {
		if (vec.at(i)->getCargo() == "Gestor")
			continue;
		if (objeto->getVolume(vec.at(i + 1)->getNome(), d1, d2)
				> objeto->getVolume(vec.at(i)->getNome(), d1, d2)) {
			std::swap(vec.at(i), vec.at(i + 1));
			troca = true;
		} else if (objeto->getVolume(vec.at(i + 1)->getNome(), d1, d2)
				== objeto->getVolume(vec.at(i)->getNome(), d1, d2)) {
			if (objeto->getFreq(vec.at(i + 1)->getNome(), d1, d2)
					> objeto->getFreq(vec.at(i)->getNome(), d1, d2)) {
				std::swap(vec.at(i), vec.at(i + 1));
				troca = true;
			}
		}
